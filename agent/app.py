...
from agent.flows import DeliveryFlow, ImageFlow, MemoryFlow, RequirementFlow
...
from agent.utils import dump_json, load_json, read_text, slugify
...
class DesignCopilotApp:
    LEAN_FULL_ONLY_DIRS = ("delivery_readiness", "workflow", "review_packet")
    ...
    def __init__(self, root: Path) -> None:
        ...
        self.requirement_flow = RequirementFlow(self)
        self.image_flow = ImageFlow(self)
        self.delivery_flow = DeliveryFlow(self)
    ...
    def run_feishu_design_cycle(
        ...
        return self.requirement_flow.run_feishu_design_cycle(
            project_key=project_key,
    ...
    def run_local_design_cycle(
        ...
        return self.requirement_flow.run_local_design_cycle(
            project_key=project_key,
            work_item_id=work_item_id,
            title=title,
            requirement_file=requirement_file,
    ...
    def _cleanup_lean_cycle_artifacts(self, output_dir: Path) -> list[str]:
        if not output_dir.exists():
            return []
        root = output_dir.resolve()
        cleaned: list[str] = []
        for name in self.LEAN_FULL_ONLY_DIRS:
    ...
    def create_delivery_readiness_report(self, project_key: str, work_item_id: str | None = None, write_markdown: bool = True) -> dict:
        return self.delivery_flow.create_delivery_readiness_report(
            project_key=project_key,
            work_item_id=work_item_id,
    ...
    def prepare_image_direction(self, project_key: str, work_item_id: str | None = None) -> dict:
        return self.image_flow.prepare_image_direction(project_key=project_key, work_item_id=work_item_id)

    def prepare_delivery_review(self, project_key: str, work_item_id: str | None = None) -> dict:
        return self.delivery_flow.prepare_delivery_review(project_key=project_key, work_item_id=work_item_id)
    ...
    def create_designer_review_packet(self, project_key: str, work_item_id: str | None = None) -> dict:
        return self.delivery_flow.create_designer_review_packet(project_key=project_key, work_item_id=work_item_id)

    def create_design_workflow_plan(self, project_key: str, work_item_id: str | None = None) -> dict:
        return self.delivery_flow.create_design_workflow_plan(project_key=project_key, work_item_id=work_item_id)
    ...
    def cockpit(
        ...
        artifact_index = self.artifact_resolver.collect_artifacts(
            target_dir if target_dir.exists() else None,
            extra_artifacts,
    ...
    def metacognition_audit(self, project_key: str) -> dict:
        return self.memory_flow.metacognition_audit(project_key)

    def create_transition_summary(self, project_key: str) -> dict:
        return self.memory_flow.create_transition_summary(project_key)

    def create_learning_digest(self, project_key: str) -> dict:
        return self.memory_flow.create_learning_digest(project_key)

    def curate_style_memory(self, project_key: str, apply: bool = False) -> dict:
        return self.memory_flow.curate_style_memory(project_key, apply=apply)
    ...
    def ingest_feedback(
        ...
        return self.memory_flow.ingest_feedback(
            project_key=project_key,
            same_category=same_category,
            work_item_id=work_item_id,
            feedback=feedback,
    ...
    def ingest_style_reference(
        ...
        return self.memory_flow.ingest_style_reference(
            project_key=project_key,
            same_category=same_category,
    ...
    def resume_session(self, project_key: str) -> dict:
        return self.memory_flow.resume_session(project_key)
