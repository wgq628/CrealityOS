...
class DesignWorkflowPlanner:
    ...
    def build(
        ...
        steps = [
            self._step("brief", "需求理解", "生成设计需求卡", "design_brief", f"python -m agent.cli build-creative-pack --project-key {project_key} --work-item-id {work_item_id}", artifact_index, payloads),
            self._clarification_step(project_key, work_item_id, artifact_index, payloads),
            self._step("creative_pack", "创作准备", "生成标准化创作包", "creative_pack", f"python -m agent.cli build-creative-pack --project-key {project_key} --work-item-id {work_item_id}", artifact_index, payloads, depends_on=["brief"]),
            self._report_step("style_transfer", "创作准备", "确认同品类风格迁移", "style_transfer_report", f"python -m agent.cli create-style-transfer-report --project-key {project_key} --work-item-id {work_item_id}", artifact_index, payloads, depends_on=["creative_pack"]),
            self._report_step("style_alignment", "创作准备", "检查风格一致性闸门", "style_alignment_report", f"python -m agent.cli create-style-alignment-report --project-key {project_key} --work-item-id {work_item_id}", artifact_index, payloads, depends_on=["creative_pack"]),
            self._step("decision_record", "创作准备", "记录设计决策依据", "design_decision_record", f"python -m agent.cli create-design-decision-record --project-key {project_key} --work-item-id {work_item_id}", artifact_index, payloads, depends_on=["creative_pack"]),
            self._step("image_batch", "出图准备", "生成多方案提示词批次", "image_generation_batch", f"python -m agent.cli create-image-production-batch --project-key {project_key} --work-item-id {work_item_id}", artifact_index, payloads, depends_on=["style_alignment"]),
            ...
            self._report_step("delivery_readiness", "交付确认", "生成交付前质量门", "delivery_readiness_report", f"python -m agent.cli create-delivery-readiness-report --project-key {project_key} --work-item-id {work_item_id}", artifact_index, payloads, depends_on=["psd_spec"], requires_confirmation=True),
